...
/**
 * Phase 2 sender: best-effort signed revalidation on publish.
 *
 ...
 */
export const revalidateChangelogWebsite: CollectionAfterChangeHook = async ({
  context,
  doc,
  previousDoc,
  req,
  ...
  if (previousDoc?._status === 'published' && previousDoc?.updatedAt === doc?.updatedAt) return doc
  ...
  try {
    ...
    const event: RevalidationEvent = {
      ...
      eventId: revalidationEventID(tenant.slug, 'changelog', slug, version),
      publishedAt,
      slug,
      tenant: tenant.slug,
    }
